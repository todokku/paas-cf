...
type_of_environment = ARGV.fetch(0)
environment_name = ARGV.fetch(1)
emit_makefile_syntax = ARGV.fetch(2, false)
...
environment = environments.dig(type_of_environment, environment_name)
raise "#{type_of_environment}/#{environment_name} not found in config" if environment.nil?
