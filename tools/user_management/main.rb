...
# Exclude aliases from USERS_CONFIG_PATH
users_config = YAML.safe_load(File.read(USERS_CONFIG_PATH), aliases:true)['users'].map { |uo| User.new(uo) }
cf_admin_users = users_config.select { |user| user.has_role_for_env?(ENV_TARGET, 'cf-admin') }
cf_auditor_users = users_config.select { |user| user.has_role_for_env?(ENV_TARGET, 'cf-auditor') }
